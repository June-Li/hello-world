...
BATCH_SIZE = 25
# tfrecord文件存放路径
TFRECORD_FILE = "V:/python-ai-3/day3/captcha/train.tfrecords"
...
def alexnet(inputs, is_training=True):
    with slim.arg_scope([slim.conv2d, slim.fully_connected],#做一次操作  下边就不用做了   用来设置范围的函数
...
with tf.Session() as sess:
    ...
    # 数据输入网络得到输出值
    logits0,logits1,logits2,logits3 = alexnet(X)
    ...
    loss2 = tf.losses.softmax_cross_entropy(one_hot_labels2, logits2)
    loss3 = tf.losses.softmax_cross_entropy(one_hot_labels3, logits3)
